...
export class JenkinsNodeEditor implements vscode.CustomTextEditorProvider {
  ...
  /** Flag anti-boucle de synchronisation — voir docs/PHASE4.md §4.7 */
  ...
  async resolveCustomTextEditor(
    ...
    const validateDisposable = bus.on('VALIDATE_REQUEST', async (msg) => {
      try {
        ...
        bus.send({ type: 'VALIDATE_RESPONSE', errors });
      } catch (err) {
    ...
    // 6. Handle RUN_REQUEST
    const runDisposable = bus.on('RUN_REQUEST', async (msg) => {
      if (!config.jenkinsUrl) {
        bus.send({ type: 'LOG_LINE', text: 'Jenkins URL not configured', level: 'error' });
        return;
      }
      try {
        ...
        bus.send({ type: 'BUILD_STARTED', buildNumber });
        for await (const text of client.streamLogs(msg.jobName, buildNumber)) {
          bus.send({ type: 'LOG_LINE', text, level: 'info' });
        }
        bus.send({ type: 'BUILD_FINISHED', buildNumber, result: 'SUCCESS' });
      } catch (err) {
        bus.send({ type: 'LOG_LINE', text: String(err), level: 'error' });
        bus.send({ type: 'BUILD_FINISHED', buildNumber: 0, result: 'FAILURE' });
      }
    });

    // 7. Handle ABORT_REQUEST
    const abortDisposable = bus.on('ABORT_REQUEST', async (msg) => {
